                    ...
                    <div id="TitleContents">
                        <form action="" method="post">
							...
							<c:if test="${(sessionScope.member.memNo eq requestScope.docu.memNo) and docu.aprType eq 'W'.charAt(0)}">
                                ...
                                <span id="apr_mod_btn"><a href="/loadAprForm.ho?docuType=O&docuNo=${docu.docuNo }">수정</a></span>
							</c:if>
                            ...
                            <fieldset id="form-done-wrap">
                                ...
                                <table id="con-info">
                                    <tr>
                                        <td>근무 구분</td>
                                        <td><c:choose>
                                        	<c:when test="${docu.ovtType eq 'P'.charAt(0) }">연장</c:when>
                                        	<c:when test="${docu.ovtType eq 'O'.charAt(0) }">야간</c:when>
                                        	<c:when test="${docu.ovtType eq 'H'.charAt(0) }">휴일</c:when>
                                        </c:choose></td>
                                    </tr>
                                    <tr>
                                        <td>근무 일시</td>
                                        <td>${docu.ovtDate }<span class="space"></span>
                                            ${docu.startHour }시 ${docu.startMinute } 분 ~ ${docu.endHour }시 ${docu.endMinute } 분</td>
                                    </tr>
                                    <tr>
                                        <td>근무 시간</td>
                                        <td>${docu.totalHour } 시간</td>
                                    </tr>
                                    <tr>
                                        <td>신청 사유</td>
                                        <td>${docu.reasons }</td>
                            	...
                            	<c:otherwise><!-- 결재가 진행중이면 -->
		                            <c:forEach var="apr" items="${aprLine}"><!-- 결재선이고, 결재를 하지 않은 경우에만 -->
		                            	<c:if test="${apr.memNo==sessionScope.member.memNo and apr.aprType =='W'.charAt(0)}">
				                            ...
				                            <div id="apr-btn-wrap">
				                            	<input type="hidden" name="docuType" value="O"/>
