...
<div id="wrap">
                        ...
                        <table id="mail-docu-list">
                            ...
                            <c:forEach var="mail" items="${requestScope.mailList.list }">
	                            <tr>
	                                ...
	                                <td><div class="mail-title"><a href="/mailView.ho?mailNo=${mail.mailNo }&other=${mail.recMemNo}&listType=F&readYN=${mail.readYN}"><c:choose><c:when test="${mail.title !=null }">${mail.title}</c:when><c:otherwise>(제목없음)</c:otherwise></c:choose></a></div></td>
	...
	<script>
        $(function(){
        	...
        	$('#categoryMail').next().children().eq(4).children().css('font-weight','800');
        	$('#categoryMail').next().children().eq(4).children().css('color','#ffcc29');
            //all 선택
         ...
         // 보관 상태 변경 처리
            $('.fa-star').click(function(){
            	var $this = $(this);
                var keepYN = '';
                var mailNo = $this.parent().prev().children().val();
                //ajax처리 필요.
                ...
                $.ajax({
                	url : "/changeKeepYN.ho",
                	data : {"keepYN": keepYN, "mailNo":mailNo, "listType":'F'},
                	type : "get",
            ...
            $('i[class*=fa-envelope]').click(function(){
                ...
                var $this = $(this);
                var readYN = '';
                var mailNo = $this.parent().prev().prev().children().val();
                ...
                $.ajax({
                	url : "/changeReadYN.ho",
                	data : {"readYN": readYN, "mailNo":mailNo, "listType":'F'},
                	type : "get",
                	success : function(result){
                		if(result){
	                		if(readYN=='Y'){
